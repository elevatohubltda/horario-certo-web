...
import { getCompany, getCompanyProperties, getCompanySchedules, registerCompanyScheduleView } from "../services/endpoints/company";
...
import SimpleScheduleFlow from "./home/SimpleScheduleFlow";
import {
  DateHeader,
  WaitlistButton,
  DialogInput,
...
export default function Home() {
  ...
  const [scheduleMode, setScheduleMode] = React.useState(null);
  ...
  const fetchScheduleMode = async () => {
    try {
      ...
      setScheduleMode(response.data?.scheduleMode || "PADRAO");
    } catch (error) {
      setScheduleMode("PADRAO");
    }
  }
  ...
  useEffect(() => {
    if (scheduleMode === "SIMPLES") return;
    handleCompanySchedules();
  ...
  return (
    <>
      ...
      {companyInfo && companyInfo.active !== false && scheduleMode === "SIMPLES" &&
        <SimpleScheduleFlow companyUrl={companyUrl} companyInfo={companyInfo} mobile={mobile} />
      }
      {companyInfo && companyInfo.active !== false && scheduleMode !== "SIMPLES" &&
...
export { DateHeader, WaitlistButton, DialogInput, Label, DialogSelect };